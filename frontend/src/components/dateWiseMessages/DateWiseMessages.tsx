            ...
            <div style={{}}>
                {
                    messages.map((message, index) => {
                        if (showMessageWithProfile(message, index, messages)) {
                            return <MessageComponent message={message} key={message.id} profile={getProfile(message, userList)} scrollIntoView={scrollIntoView && (index === messages.length - 1)} />
                        } else {
                            return <MessageComponent message={message} key={message.id} scrollIntoView={scrollIntoView && (index === messages.length - 1)} />
                        }
                    })
                }
            </div>
